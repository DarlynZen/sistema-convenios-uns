	...
	<div class="space-y-4">
		...
		<x-admin.block>
			<div class="flex flex-col gap-2">
				<h1 class="text-neutral-600 text-2xl font-bold">Editar convenio</h1>
				<p class="text-neutral-600 text-sm">Actualiza la informacion del convenio y sus documentos</p>
			</div>
		</x-admin.block>
		...
		<div class="bg-white rounded border border-neutral-400">
			<x-admin.stepper :steps="[
				...
				['key' => 'details', 'label' => 'Institucion', 'description' => 'Entidad y coordinadores', 'icon' => $stepperIconInstitucion],
				['key' => 'docs', 'label' => 'Documentacion', 'description' => 'Archivos PDF requeridos', 'icon' => $stepperIconDocumentos],
			]" current="1">
				<form id="form-editar-convenio" method="POST"
					action="{{ route('admin.convenios.update', $convenio) }}"
					enctype="multipart/form-data" class="space-y-3">
					@csrf
					@method('PATCH')

					{{-- Seccion: Informacion principal del convenio (Paso 1) --}}
					<section class="w-full" x-show="step === 1" x-cloak data-step-section="1">
						<div class="p-5">
							<div class="mb-4 flex items-start gap-2">
								...
								<div class="min-w-0">
									<h2 class="text-sm font-semibold text-neutral-800">Informacion del Convenio</h2>
									<p class="mt-0.5 text-xs text-neutral-500">Actualiza los datos generales y la
							...
							<div class="space-y-4">
								<div>
									<x-input-label for="titulo" value="Titulo del convenio" />
									<textarea id="titulo" name="titulo" rows="4" required
										...
										placeholder="Nombre del titulo del convenio">{{ old('titulo', $convenio->titulo) }}</textarea>
								...
								<div class="grid grid-cols-1 md:grid-cols-4 gap-4">
									<div>
										...
										<select id="tipo_convenio_id" name="tipo_convenio_id" required
											...
											@foreach ($tiposConvenio as $tipo)
												<option value="{{ $tipo->id }}" @selected(old('tipo_convenio_id', $convenio->tipo_convenio_id) == $tipo->id)>
													{{ $tipo->nombre }}
												</option>
											@endforeach
										</select>
									...
									<div>
										<x-input-label for="ambito_id" value="Tipo de ambito" />
										<select id="ambito_id" name="ambito_id" required
											class="mt-1 block w-full rounded-lg border border-neutral-300 px-3 py-2 text-[13px] focus:border-red-400 focus:ring-2 focus:ring-red-200">
											<option value="">Seleccionar ambito</option>
											@foreach ($ambitos as $ambito)
												<option value="{{ $ambito->id }}" @selected(old('ambito_id', $convenio->ambito_id) == $ambito->id)>
													{{ $ambito->nombre }}
												</option>
											@endforeach
										</select>
									...
									<div>
										<x-input-label for="resolucion" value="Nro. de Transcripcion" />
										<x-text-input id="resolucion" name="resolucion" type="text"
											class="mt-1 block w-full rounded-lg border border-neutral-300 px-3 py-2 text-[13px] focus:border-red-400 focus:ring-2 focus:ring-red-200"
											placeholder="Nro. de transcripcion" :value="old('resolucion', $convenio->resolucion)" />
								...
								<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
									<div>
										...
										<x-text-input id="nacionalidad" name="nacionalidad" type="text"
											class="mt-1 block w-full rounded-lg border border-neutral-300 px-3 py-2 text-[13px] focus:border-red-400 focus:ring-2 focus:ring-red-200"
											placeholder="Nombre del pais" :value="old('nacionalidad', $convenio->nacionalidad)" />
									...
									<div>
										...
										<select id="estado_convenio_id" name="estado_convenio_id" required
											...
											@foreach ($estadosConvenio as $estado)
												<option value="{{ $estado->id }}" @selected(old('estado_convenio_id', $convenio->estado_convenio_id) == $estado->id)>
													{{ $estado->nombre }}
												</option>
											@endforeach
										</select>
								...
								<div>
									@php
										$beneficiariosOptions = collect($beneficiarios ?? [])
											...
											->values();
										$beneficiariosSelected = collect(old('beneficiarios',
											collect(data_get($convenio, 'beneficiarios', data_get($convenio, 'beneficiario', [])))
												->pluck('id')
												->all()
										))
									...
									<x-admin.beneficiarios-multiselect
										...
										:selected="$beneficiariosSelected"
								...
								<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
									<div>
										<x-input-label for="fecha_inicio" value="Fecha de suscripcion" />
										<x-text-input id="fecha_inicio" name="fecha_inicio" type="date"
											class="mt-1 block w-full rounded-lg border border-neutral-300 px-3 py-2 text-[13px] focus:border-red-400 focus:ring-2 focus:ring-red-200"
											required :value="old('fecha_inicio', optional($convenio->fecha_inicio)->format('Y-m-d'))" />
										<x-input-error :messages="$errors->get('fecha_inicio')" class="mt-1" />
									</div>

									<div>
										<x-input-label for="duracion_valor" value="Duracion" />
										<div class="mt-1 grid grid-cols-2 gap-2">
											<x-text-input id="duracion_valor" name="duracion_valor" type="number"
												...
												placeholder="Valor" :value="old('duracion_valor', $convenio->duracion_valor)" />
											<select id="duracion_unidad" name="duracion_unidad" required
												...
												<option value="dias" @selected(old('duracion_unidad', $convenio->duracion_unidad) === 'dias')>Dias</option>
												<option value="semanas" @selected(old('duracion_unidad', $convenio->duracion_unidad) === 'semanas')>Semanas</option>
												<option value="meses" @selected(old('duracion_unidad', $convenio->duracion_unidad) === 'meses')>Meses</option>
												<option value="anios" @selected(old('duracion_unidad', $convenio->duracion_unidad) === 'anios')>Anios</option>
											</select>
										</div>
									...
									<div>
										<x-input-label for="plazo_prorroga_valor" value="Plazo de prorroga" />
										<div class="mt-1 grid grid-cols-2 gap-2">
											<x-text-input id="plazo_prorroga_valor" name="plazo_prorroga_valor"
												type="number" min="1" step="1" required
												class="block w-full rounded-lg border border-neutral-300 px-3 py-2 text-[13px] focus:border-red-400 focus:ring-2 focus:ring-red-200"
												placeholder="Valor" :value="old('plazo_prorroga_valor', $convenio->plazo_prorroga_valor)" />
											<select id="plazo_prorroga_unidad" name="plazo_prorroga_unidad" required
												class="block w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-[13px] focus:border-red-400 focus:ring-2 focus:ring-red-200">
												<option value="">Unidad</option>
												<option value="dias" @selected(old('plazo_prorroga_unidad', $convenio->plazo_prorroga_unidad) === 'dias')>Dias</option>
												<option value="semanas" @selected(old('plazo_prorroga_unidad', $convenio->plazo_prorroga_unidad) === 'semanas')>Semanas</option>
												<option value="meses" @selected(old('plazo_prorroga_unidad', $convenio->plazo_prorroga_unidad) === 'meses')>Meses</option>
											</select>
										</div>
					...
					{{-- Seccion: Informacion de la Institucion colaboradora (Paso 2) --}}
					<section class="space-y-3" x-show="step === 2" x-cloak data-step-section="2">
						<div class="p-4 sm:p-5">
							<div class="mb-4 flex items-start gap-2">
								...
								<div class="min-w-0">
									<h2 class="text-sm font-semibold text-neutral-800">Informacion de la Institucion
										colaboradora</h2>
									<p class="mt-0.5 text-xs text-neutral-500">Datos de la institucion externa,
							...
							<div class="space-y-3">
								<div>
									<x-input-label for="entidad_nombre" value="Institucion/Entidad/Organismo" />
									<x-text-input id="entidad_nombre" name="entidad_nombre" type="text"
										class="mt-1 block w-full rounded-lg border border-neutral-300 px-3 py-2 text-[13px] focus:border-red-400 focus:ring-2 focus:ring-red-200"
										placeholder="Nombre de la institucion" required
										:value="old('entidad_nombre', $convenio->entidad_nombre)" />
								...
								<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
									<div>
										...
										<x-text-input id="entidad_tipo" name="entidad_tipo" type="text"
											class="mt-1 block w-full rounded-lg border border-neutral-300 px-3 py-2 text-[13px] focus:border-red-400 focus:ring-2 focus:ring-red-200"
											placeholder="Tipo de entidad" :value="old('entidad_tipo', $convenio->entidad_tipo)" />
								...
								@php
									$tipoConvenioEspecifico = $tiposConvenio?->where('nombre', 'especifico')->first();
								...
								<div x-data="{
									...
									<x-input-label for="observaciones_prorroga" value="Observacion" />
									...
									<textarea id="observaciones_prorroga" name="observaciones_prorroga" rows="3"
										...
										placeholder="Observacion sobre el convenio especifico...">{{ old('observaciones_prorroga', $convenio->observaciones_prorroga) }}</textarea>
								...
								<div class="rounded-lg border border-neutral-300 bg-neutral-50/60 p-4">
									<div class="mb-4 flex items-start justify-between gap-3">
										<div>
											...
											<p class="mt-0.5 text-xs text-neutral-500">Coordinador de la UNS y de la
												institucion externa.</p>
										</div>
									</div>

									@php
										$coordinadoresJson = data_get($convenio, 'detalles_coordinadores_json', []);
										$coordinadoresUnsOld = collect(old('coordinador_uns', data_get($coordinadoresJson, 'coordinador_uns', [])))->values();
										$coordinadoresInstOld = collect(old('coordinador_institucion', data_get($coordinadoresJson, 'coordinador_institucion', [])))->values();
										$noSeMenciona = (bool) old('no_se_menciona', data_get($coordinadoresJson, 'no_se_menciona', false));
										$coordinadoresCount = max($coordinadoresUnsOld->count(), $coordinadoresInstOld->count(), 1);
										$coordinadoresIniciales = collect(range(0, $coordinadoresCount - 1))
											->map(fn($index) => [
												'coordinador_uns' => $coordinadoresUnsOld->get($index, ''),
												'coordinador_institucion' => $coordinadoresInstOld->get($index, ''),
									...
									<div x-data="{
										coordinadores: @js($coordinadoresIniciales),
										agregarCoordinador() {
											...
											<div
												...
												<div class="flex-1">
													...
													<x-text-input name="coordinador_uns[]" type="text"
														...
														x-model="coord.coordinador_uns" />
												</div>

												<div class="flex-1">
													<x-input-label value="Coordinador Institucion" />
													<x-text-input name="coordinador_institucion[]" type="text"
														class="mt-1 block w-full rounded-lg border border-neutral-300 px-3 py-2 text-[13px] focus:border-red-400 focus:ring-2 focus:ring-red-200"
														placeholder="Nombre del coordinador institucion"
														x-model="coord.coordinador_institucion" />
												</div>
												...
												<div class="flex items-end gap-2">
													<label
														...
														<input type="checkbox" name="no_se_menciona" value="1"
															class="rounded-lg border-neutral-300" x-model="coord.no_se_menciona" />
														No se menciona
													</label>
												</div>
									...
									<p class="mt-2 text-xs text-neutral-500">Puedes agregar multiples coordinadores o
								...
								<details class="rounded-lg border border-neutral-300 bg-neutral-50/60">
									...
									<div class="px-3 pb-3">
										<p class="text-xs text-neutral-500">Si aplica, describe el objetivo del
											convenio especifico.</p>
										<textarea id="objetivo_personalizado" name="objetivo_personalizado" rows="3"
											class="mt-2 block w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-[13px] focus:border-red-400 focus:ring-2 focus:ring-red-200"
											placeholder="Describe el objetivo del convenio.">{{ old('objetivo_personalizado', $convenio->objetivo_personalizado) }}</textarea>
					...
					{{-- Seccion: Archivos PDF / Documentos (Paso 3) --}}
					<section class="space-y-3" x-show="step === 3" x-cloak data-step-section="3">
						<div class="p-4 sm:p-5">
							<div class="mb-4 flex items-start gap-2">
								...
								<div class="min-w-0">
									<h2 class="text-sm font-semibold text-neutral-800">Documentacion</h2>
							...
							<div class="space-y-3">
								<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
									<x-admin.file-dropzone label="Resolucion (PDF)" name="archivo_uno" />
									<x-admin.file-dropzone label="Convenio (PDF)" name="archivo_dos" />
								</div>
									...
									<div class="flex items-start gap-3">
										<div>
											...
											<p class="mt-1 text-sm text-neutral-600">Verifica que los documentos sean
												legibles y correspondan al convenio descrito en los pasos anteriores.</p>
										</div>
									</div>
								</div>
							</div>
